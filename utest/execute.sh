...
shellNS_utest_execute() {
  ...
  if [ "${isOk}" != "1" ]; then
    shellNS_main_boot_dialog "error" "ShellNS-UTest not found!"
    return 1
  fi


  local -a arrayPackageFiles=()
  shellNS_main_boot_getFiles "${SHELLNS_TMP_PACKAGE_DIR_PATH}" "arrayPackageFiles" "utest"
  ...
  if [ "${#SHELLNS_UTEST_MAIN_MAPPING[@]}" == "0" ]; then
    shellNS_main_boot_dialog "error" "No tests were found"
    return 1
  fi
