...
shellNS_standalone_install() {
  ...
  if [ "${strContext}" != "install" ] && [ "${strContext}" != "update" ]; then
    ...
    shellNS_main_boot_dialog "error" "${strMessageContent}"
    return 1
  fi
  ...
  shellNS_main_boot_dependencies "${isAutoLoadPendendencies}"; statusSet "$?"
  if [ $(statusGet) != "0" ]; then
    intReturn="1"
  ...
  shellNS_main_boot_dialog "${strMessageType}" "${strMessageContent}"
  return "${intReturn}"
}