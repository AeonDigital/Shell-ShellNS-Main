...
shellNS_standalone_export() {
  ...
  shellNS_main_boot_getFiles "${SHELLNS_TMP_PACKAGE_DIR_PATH}" "arrayExportFiles" "export"
  ...
  if [ "$?" != "0" ]; then
    shellNS_main_boot_dialog "error" "Error on create 'standalone/package.sh' file."
  fi
  shellNS_main_boot_dialog "ok" "File 'standalone/package.sh' created."
  return 0
}